...
export function CTASection({ smoothX, smoothY }: CTAProps) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-[1600px] mx-auto px-6 lg:px-12 text-center flex flex-col items-center justify-center">
        ...
        <div className="overflow-hidden mb-2 sm:mb-3">
          <motion.h2 initial={{ y: "110%", opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 1.3, ease: ease }} className="font-syne text-text-primary tracking-tighter text-[clamp(2.4rem,5.5vw,6.4rem)] font-bold leading-[1.04]">Let's Build Something</motion.h2>
        </div>
        <div className="overflow-hidden mb-8 sm:mb-12">
          <motion.h2 initial={{ y: "110%", opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 1.3, delay: 0.1, ease: ease }} className="font-syne text-brand-primary tracking-tighter italic font-light text-[clamp(2.4rem,5.5vw,6.4rem)] leading-[1.04]">Extraordinary</motion.h2>
        </div>
        <motion.p initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 1, delay: 0.25, ease: ease }} className="font-dm text-text-secondary max-w-2xl mx-auto mb-10 sm:mb-14 text-[clamp(17px,2.1vw,22px)] leading-[1.75]">
